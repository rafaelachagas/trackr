...
import { RefreshCw, BarChart2, LogOut, Eye, EyeOff, Link2, CheckCircle2 } from 'lucide-react'
...
export default function AdAccountsPage() {
  ...
  const connectMetaOAuth = useCallback(() => {
    ...
    const handler = (event: MessageEvent) => {
      ...
      if (event.data?.type === 'meta_auth_success') {
        ...
        if (accounts.length === 1) {
          setAdAccountId(accounts[0].id.replace('act_', ''))
        }
        carregar()
      } else if (event.data?.type === 'meta_auth_error') {
        window.removeEventListener('message', handler)
        setMetaConectando(false)
        alert(`Connection error: ${event.data.error}`)
      }
    }
  ...
  async function syncMeta() {
    ...
    try {
      ...
      alert(`Sync complete! ${json.total_registros} records imported (last ${diasSync} days).`)
    } catch (e: any) {
  ...
  return (
    <div className="max-w-4xl mx-auto text-slate-200 pb-12">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Ad Accounts</h1>
          <p className="text-slate-400 text-sm mt-1">Manage your advertising accounts</p>
        </div>
        <button
          onClick={carregar}
          disabled={loading}
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Connect and manage accounts</p>
            </div>
          </div>
          ...
          {metaAccessToken ? (
            ...
            >
              ...
              {metaConectando ? 'Opening Facebook...' : 'Login with Facebook'}
            </button>
          )}
        </div>
        ...
        <div className="px-6 py-5">
          {!metaAccessToken ? (
            ...
            <div className="space-y-3">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Select ad account</p>
              <select
                value={adAccountId}
            ...
            <div className="flex items-center justify-between p-3 bg-slate-800/40 border border-slate-700/50 rounded-xl">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm font-medium text-slate-200">act_{adAccountId}</span>
              </div>
              <span className="text-xs text-slate-500">Active account</span>
            </div>
          ) : (
            <div className="space-y-3">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Ad Account ID</p>
              <input
                type="text"
                value={adAccountId}
                onChange={e => setAdAccountId(e.target.value)}
              ...
              />
              <p className="text-[10px] text-slate-500">Numbers only, without "act_".</p>
              <button onClick={saveAccount} disabled={saving || !adAccountId} className="w-full py-2.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold transition disabled:opacity-50">
                {saving ? 'Saving...' : 'Save account'}
              </button>
            </div>
          )}
          ...
          {metaAccessToken && (
            <details className="mt-4 group">
              <summary className="text-[10px] text-slate-600 cursor-pointer hover:text-slate-400 transition select-none">Advanced: view/edit token manually</summary>
              <div className="mt-2 relative">
                ...
                />
                <button type="button" onClick={() => setShowToken(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition">
