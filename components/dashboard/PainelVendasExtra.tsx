...
function VendasPorProduto({ data, isPrivate }: { data: VendasBreakdown | null; isPrivate: boolean }) {
...
function VendasPorPagamento({ data, isPrivate }: { data: VendasBreakdown | null; isPrivate: boolean }) {
  const metodos = data?.porPagamento ?? []
  // Só faz sentido mostrar quando há método de fato registrado (não só "Não informado").
...
export default function PainelVendasExtra() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <VendasPorProduto data={data} isPrivate={isPrivate} />
      <VendasPorPagamento data={data} isPrivate={isPrivate} />
    </div>
  )
}
