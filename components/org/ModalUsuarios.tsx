...
import { X, UserPlus, Copy, Check, Trash2, Shield, User, ChevronDown, Link2, Loader2, Crown } from 'lucide-react'
...
export default function ModalUsuarios({ activeOrg, currentUserId, onClose }: Props) {
  ...
  async function generateInvite() {
    ...
    setGenerating(false)
    if (json.token) {
      setInviteLink(`${window.location.origin}/convite/${json.token}`)
    }
  }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
          <div>
            <p className="text-[9px] font-black uppercase tracking-widest text-muted-foreground">Organização</p>
            <h2 className="text-base font-black text-foreground tracking-tight">{activeOrg.org_name}</h2>
          </div>
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
          ...
          {/* Gerar link de convite — só admin */}
          {isAdmin && (
            <div className="space-y-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Convidar membro</p>
              ...
              <div className="flex gap-2">
                ...
                <div className="relative">
                  <button
                    ...
                    style={{ backgroundColor: '#1a2022', border: '1px solid rgba(255,255,255,0.07)', color: '#e2e8f0' }}
                  >
                  ...
                  {roleMenuOpen === 'new' && (
                    <div className="absolute top-full mt-1 left-0 z-20 rounded-xl p-1 w-32 shadow-xl" style={{ backgroundColor: '#1a2022', border: '1px solid rgba(255,255,255,0.07)' }}>
              ...
              {inviteLink && (
                <div
                  ...
                  style={{ backgroundColor: '#1a2022', border: '1px solid rgba(0,174,239,0.2)' }}
                >
                  <p className="flex-1 text-[10px] font-mono text-muted-foreground truncate">{inviteLink}</p>
                  <button
                    onClick={copyLink}
                    className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider transition"
              ...
              <p className="text-[9px] text-muted-foreground">O link expira em 7 dias e só pode ser usado uma vez.</p>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            {loading ? (
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center font-black text-[10px] text-white flex-shrink-0">
                  {initials(member)}
                </div>
                ...
                {isAdmin && member.user_id !== currentUserId ? (
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider transition hover:bg-white/5"
                    ...
                    {roleMenuOpen === member.user_id && (
                      <div className="absolute right-0 top-full mt-1 z-20 rounded-xl p-1 w-32 shadow-xl" style={{ backgroundColor: '#13181a', border: '1px solid rgba(255,255,255,0.07)' }}>
                        {(['member', 'admin'] as const).map(r => (
                          <button
                            key={r}
                  ...
                  <span
                    ...
                    style={{ color: member.role === 'admin' ? '#f59e0b' : '#6b7980', backgroundColor: member.role === 'admin' ? 'rgba(245,158,11,0.1)' : 'rgba(107,121,128,0.1)' }}
                  >
                    {member.role === 'admin' ? 'Admin' : 'Membro'}
                  </span>
                )}
