...
type Preset = 'Hoje' | 'Ontem' | 'Últimos 7 dias' | 'Últimos 30 dias' | 'Todo o Período'
const PRESETS: Preset[] = ['Hoje', 'Ontem', 'Últimos 7 dias', 'Últimos 30 dias', 'Todo o Período']

function rangeDoPreset(p: Preset): Range {
...
// ---------- Seletor de data (igual ao da VTurb) ----------
function SeletorData({ range, onChange }: { range: Range; onChange: (r: Range) => void }) {
  ...
  const presetAtivo = PRESETS.find((p) => { const r = rangeDoPreset(p); return r.ini === range.ini && r.fim === range.fim })
  const hoje = hojeSP()
